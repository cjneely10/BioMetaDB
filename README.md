...
- `Record` - all objects generated by the database
    _ `__repr()__`:         returns a "pretty-print" summary of the record
    - `full_path()`:        returns full path to data file
    - `write()`:            writes entire contents of data file to current directory as temp file
    - `clear()`:            deletes temporary file
    - `print()`:            prints contents of data file to screen
    - `get_records()`:      calls SeqIO.parse() from the Bio package (optimized for larger files)
    
- `RecordList` - returned by `get_table()`, a quasi list/dict data type for multiple records.
    - `query(query_str)`:   returns a list of records matching query, or all records if no query passed
